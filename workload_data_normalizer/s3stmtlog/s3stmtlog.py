...
def stmt_sample_user(users):
    for u in users:
        return escape_single_quote(u)
    return None

def stmt_index_names(index_names):
    if index_names == None:
        return ''
    return escape_single_quote(",".join(index_names))
...
def s3stmt2sql(one_stmt_json):
    ...
    cols = {
        ...
        "DIGEST_TEXT": escape_single_quote(d['normalized_sql']),
        "SAMPLE_USER": stmt_sample_user(d['auth_users']),
        "QUERY_SAMPLE_TEXT": escape_single_quote(d['sample_sql']),
    ...
    for col in str_cols:
        cols[col] = "'" + escape_single_quote(d[col.lower()]) + "'"
